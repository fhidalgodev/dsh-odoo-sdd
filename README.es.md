...
2. **ARCHITECTURE** — diseña modelos/vistas/seguridad en `architecture.md` + `test-plan.md`; busca primero funcionalidad existente; gate `APPROVED`. El arquitecto **pregunta** por los **tipos de vista extra** más allá de form/tree (kanban, pivot, graph, calendar, dashboard, …) y por los **reportes** (PDF vía `ir.actions.report`/QWeb, SQL, CSV/XLSX, herramienta externa), declarando "form + tree only" / "no reports needed" cuando corresponda. `## Views` y una sección `## Reports` llevan estas decisiones; son decisiones **guía** (no bloqueantes, se muestran como avisos en `sdd_phase status`) — el modelo de seguridad sigue siendo el único bloqueante fail-closed.
